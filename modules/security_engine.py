...
class AuditPedigree(BaseModel):
    """
    Cryptographic proof (Pedigree Pattern) of the audit.
    Attests that the decision was generated by a specific model version
    given a specific context.
    """
    context_hash: str
    model_version: str
    signature: str
    attestation_type: str = "HMAC-SHA256"
...
class DecisionOutput(BaseModel):
    ...
    processing_time_ms: int
    timestamp: str
    pedigree: Optional[AuditPedigree] = None
    tribunal_notes: Optional[dict] = None
...
class DecisionEngine:
    def _generate_pedigree(
        ...
        """Generates a cryptographic attestation of the audit (Pedigree Pattern)."""
        ctx_payload = {
            "agent_id": agent_id,
            "risk_score": round(risk.risk_score, 4),
            "rules": sorted(risk.rules_triggered),
            "amount": amount,
            "destination": dest.lower()
        }
        ctx_json = json.dumps(ctx_payload, sort_keys=True)
        ctx_hash = hashlib.sha256(ctx_json.encode()).hexdigest()

        # In a production environment, this signature would be generated
        # inside a TEE (AMD SEV-SNP) or via an HSM.
        # Here we simulate the pedigree signature for the 'Imina-Na' model.
        secret_key = "imina-na-internal-audit-key"
        import hmac
        sig = hmac.new(
            secret_key.encode(),
            ctx_hash.encode(),
        ...
        return AuditPedigree(
            context_hash=ctx_hash,
            model_version="Imina-Na-V2-LoRA",
            signature=sig
        )
    ...
    async def decide(
        ...
        if risk.hard_block:
            ...
            # Note: _decide_with_ai could return additional data like tribunal_notes
            # Refactoring it slightly here if needed, but let's keep it simple.
            # In current implementation, _decide_with_ai calls crewai and others.

            # Temporary manual call to get tribunal_notes if crewai is used
            ai_decision = None
            ai_status = "none"

            if settings.decision_ai_enabled and llm_gateway.is_available():
                payload = {
                    "agent_id": agent_id, "action_type": action_type,
                    "amount_usdc": amount_usdc, "destination": destination,
                ...
                crew_result = await crew_decision_brain.decide(payload)
                if crew_result:
                    ai_decision, ai_status = Decision(crew_result["decision"]), "ai_crewai"
                    tribunal_notes = crew_result.get("tribunal_notes")

            if not ai_decision:
                ai_decision, ai_status = await self._decide_with_ai(
                    agent_id, action_type, amount_usdc, destination,
                    risk, sigui_mode, escalation_available, agent_profile
                )
        ...
        # Generate Pedigree (Pedigree Pattern)
        pedigree = self._generate_pedigree(agent_id, risk, amount_usdc, destination)
