...
> [!IMPORTANT]
> **Research Status Notice** — Trustformer (T-GAT) is a *theoretical architectural contribution currently under validation*. The <5ms latency figure is a **projected** estimate derived from sparse-attention complexity analysis and has **not yet been validated** on production hardware. The current production model for Sigui Protocol is **Imina-Na V2** (Qwen2-VL-7B-Instruct + LoRA r=16, α=32, available at [Ibonon/Imina-Na-V2](https://huggingface.co/Ibonon/Imina-Na-V2)), which operates at <50ms on AMD MI300X and represents the battle-tested production oracle.
